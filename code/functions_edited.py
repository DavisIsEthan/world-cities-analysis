...
def plot_dists_and_neighbors(cities, centroids, scaled):
    ...
    rev_clust = dict((v, k) for k, v in clust.items())
    for j, c in centroids.iterrows():
        ...
        for i, row in scaled[scaled['Cluster'] == rev_clust[j+1]].iterrows():
            ...
            for k, c2 in centroids.iterrows():
                ...
                if d < min_dist:
                    ...
                    neighbors[i] = k+1
            ncss[j] += min_dist**2
        ncss[j] = ncss[j] / len(scaled[scaled['Cluster'] == rev_clust[j+1]])
        count[j] = len(scaled[scaled['Cluster'] == rev_clust[j+1]])
    pd.merge(cities[['City', 'Region', 'Cluster']], pd.DataFrame(city_dists, columns=cities['City']),
